...
export default function Vocabulary(cb) {
  ...
  const makeHardWords = async (arr) => {
    ...
    if (hardWordsArr.length) {
      ...
      hardWordsHtml += '<li class="vocabulary-word-item">Нету сложных слов</li>';
    }
  ...
  const makeDeleteWords = async (arr) => {
    ...
    if (hardWordsArr.length) {
      ...
      deleteWordsHtml += '<li class="vocabulary-word-item">Нету удаленных слов</li>';
    }
  ...
  const addEventListeners = () => {
    ...
    containerRef.querySelector('.vocabulary-delete-words-list').addEventListener('click', (e) => {
      if (e.target.classList.contains('vocabulary-page-btn')) {
        removeWord(e);
        deleteWordsArr.splice(e.target.dataset.index, 1);
        makeDeleteWords(hardWordsArr);
      }
    });
  };
